...
export default async function crearEvento(data: FormDataEvento, usuarioId: number) {
  try {
    ...
    if (data.fotoHorizontal) {
      horizontalBlobName = generateImageName(data.fotoHorizontal, 0, 'horizontal');
    ...
    if (data.fotoHorizontal && horizontalBlobName) {
      ...
      uploadPromises.push(
        uploadEventImage(data.fotoHorizontal, eventoCreado.eventoId, 'horizontal', horizontalBlobName)
          .then(blobName => {
    ...
    if (data.fotoVertical && verticalBlobName) {
      ...
      uploadPromises.push(
        uploadEventImage(data.fotoVertical, eventoCreado.eventoId, 'vertical', verticalBlobName)
          .then(blobName => {
    ...
    // 5. Si hay imagen de zonas, subirla
    if (eventoCreado && data.imagenZonasFile && zonasBlobName) {
      console.log("📤 Subiendo imagen de zonas...");
      
      try {
        await uploadEventImage(data.imagenZonasFile, eventoCreado.eventoId, 'zona', zonasBlobName);
